...
MAX_OUT_DEGREE_FOR_SOURCE = 1000
MAX_OUT_DEGREE_ALLOWED = 100000
...
def find_safe_source(G, max_out=MAX_OUT_DEGREE_FOR_SOURCE):
    print("🔍 Looking for alternative safe source node...")
    for node in G.nodes:
        if G.out_degree(node) < max_out and G.nodes[node].get("node_type") == "substance":
            print(f"✅ Using {node} as safe source (out-degree: {G.out_degree(node)})")
            return node
    print("❌ No suitable safe source node found.")
    return None

def graph_to_filtered_dataframe(G: nx.DiGraph):
    nodes = list(G.nodes())
    ...
    for u, v in G.edges():
        if G.out_degree(u) < MAX_OUT_DEGREE_ALLOWED:
            if u in node_to_idx and v in node_to_idx:
                edges.append((node_to_idx[u], node_to_idx[v]))
        else:
            skipped += 1

    print(f"Filtered out {skipped} edges from high-degree hubs")
    edges_df = pd.DataFrame(edges, columns=['source', 'target'])
    print(f"Edges DataFrame created: {edges_df.shape[0]} rows")
    print(edges_df.head())
    log_memory("After filtering edges and creating DataFrame")
    return edges_df, node_to_idx, idx_to_node

def find_shortest_path_graphblas(G: nx.DiGraph, source: str, target: str) -> Optional[Dict]:
    edges_df, node_to_idx, idx_to_node = graph_to_filtered_dataframe(G)
    n_nodes = len(G.nodes())
    ...
    while frontier.nvals > 0 and not found:
        distance += 1
        print(f"\n--- Step {distance} ---")
        print(f"Frontier nodes: {list(frontier.to_coo()[0])}")
        ...
        print(f"Next frontier after masking: {list(next_frontier.to_coo()[0])}")
...
def main():
    ...
    if G.out_degree(SOURCE_NODE) > MAX_OUT_DEGREE_FOR_SOURCE:
        print(f"⚠️ Source node is a hub (out-degree: {G.out_degree(SOURCE_NODE)}). Trying to find another...")
        source_node = find_safe_source(G)
        if not source_node:
            return
